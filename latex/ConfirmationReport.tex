...
[K^{\pm}\pi^{\mp}]_D\pi^0/\gamma)\pi^{\pm}/K^{\pm}$ decays, in order to
understand the background composition. The fitting region lies within the
ranges 5050 $<$ $m[D^*h]$ $<$ 5800 MeV/$c^2$ and 50 (135) $<$ $\Delta_M$ $<$ 210
MeV/$c^2$ for the $D\gamma$ ($D\pi^0$) final states. 

The physics backgrounds that fall within this region are given in table ??. They
can be split into three categories (as labelled in the table):
\begin{enumerate}
...
\begin{itemize}
\item List backgrounds known to contaminate signal.
\item Backgrounds have unknown/unpredictable charge asymmetries. 1D fit didn't
work as suffered from fit instabilities with shapes and asymmetries floating, or
large systematics with backgrounds fixed from simulation.
\item Backgrounds fall into 5 categories: partially reconstructed, partially
reconstructed cross-feed, cross-feed, mis-reconstructed, over-reconstructed.
\item Signal is the only component that peaks in both B and Delta mass - hope
this will constrain the signal yield to protect against unpredictable charge
asymmetries.
\\
\item Describe correlation with 2D (1D) polynomial.
...
\begin{itemize}
\item Table of yields and expected yields from MC efficiencies
