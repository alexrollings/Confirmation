...
are constructed from polynomial functions of $\Delta_M$:  
\begin{equation}
...
using these polynomial coefficients. Looking at the 2D plots of the
data/simulation samples shown in Fig. ??, it can be seen that these variables
exhibit greater correlation in the $D\gamma$ final state than in the $D\pi^0$
final state. This is due to the extra constraint on the $\pi^0$ mass in
calculating the re-fitted $m[D^*h]$ mass variable during reconstruction, as
described at the beginning of Sec.~\ref{sec:selections}. In the 2D fits to
simulation for the $D^*\rightarrow D\pi^0$ mode, a linear polynomial is found to
be sufficient to model this correlation.
