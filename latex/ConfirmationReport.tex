...
\usepackage[left=2.5cm,right=2.5cm,top=2cm,bottom=2.4cm]{geometry} 
\usepackage{sidecap}
\usepackage{fullpage}
\geometry{a4paper}
...
\hline
\multirow{2}{*}{\textbf{Decay Mode}} & \multicolumn{2}{c|}{\textbf{Run1 Yield (3
fb$^{-1}$)}} & \multicolumn{2}{c|}{\textbf{2011-2017 Yield (6.6 fb$^{-1}$)}} \\ \cline{2-5} 
...
% By studying $B^{\pm}\rightarrow D^*\pi^{\pm}$ decays, a selection procedure has
% been finalised for both $D^*$ final states, $D\pi^0$ and $D\gamma$. Due to the
% lack of success from pursuing a 1D mass fit to the $m[D^*h]$ spectrum, a 2D fit
% is now under development in order to further constrain the signal yields using
% the extra information in the $\Delta_M$ distribution. Improvements to this fit
% will be implemented in the immediate future, aided by the generation of Run2
% simulation samples of physics backgrounds and fit stability studies.
% Currently, measurements of the physics observable $R^{K\pi ,
% \pi^0/\gamma}_{K/\pi}$ are consistent with previous studies and expected
% sensitivity to events in the channels sensitive to $\phi_3$ are promising. 
...
\end{scope}
\end{tikzpicture}
\caption{{$\Delta_M$ vs. $m[D^*h]$ distributions of (a) Run1 data, (b)
...
\end{tikzpicture}
\caption{{1D projections onto the $m[D^*h]$ (left) and $\Delta_M$ (right)
...
\noindent Study of $B^{\pm}\rightarrow D^{*}K^{\pm}$ decays using the full LHCb
dataset. \\
...
achieve this by Easter 2018. Plan to carry out the optimisation and evaluate
systematics by summer 2018.
