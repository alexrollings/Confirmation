...
topology of the type $B^{\pm}\rightarrow Dh^{\pm}$. A neutral particle
is then combined with the $D$ meson to make a $B^{\pm}\rightarrow D^*h^{\pm}$
candidate. A kinematic fit is performed to the
entire decay chain of each candidate, with vertex constraints applied to both
the $B^{\pm}$ and $D$ daughters, and $D$ and $\pi^0$ (when present) mesons are
...
The offline selection consits of 2 stages of multivariate analysis (MVA) and
rectangular cuts, in order to reduce combinatorial and physics backgrounds.

The combinatoric backgrounds we are trying to suppress during the first stage
MVA are fake $D$ candidates, formed from the combination of random tracks
disguised as $D$ daughters, and fake $B$ candidates, made up of a $D$ meson
plus a random track.  The use of Boosted Decision Trees
(BDTs)~\cite{RegressionTrees} in isolating $B^{\pm}\rightarrow
[h^{\pm}h^{\mp}]_{D}h^{\pm}$ decays has been well established by the 2-body
ADS/GLW analysis of this mode~\cite{B2DKD2hh}. We therefore employ a similar
strategy here. 
...
\begin{scope}[x={(image.south east)},y={(image.north west)}]
\draw[green,ultra thick] (0.533,0.29) rectangle (0.90,0.90);
\node[green,ultra thick] at (0.72,0.6) {BDT1};
\draw[red,ultra thick] (0.20,0.50) rectangle (0.42,0.90);
\node[red,ultra thick] at (0.31,0.7) {BDT2};
\end{scope}
\end{tikzpicture}
\begin{tikzpicture}[scale=0.48]
\node[anchor=south west,inner sep=0] (image) at (0.5,0)
...
\begin{scope}[x={(image.south east)},y={(image.north west)}]
\draw[green,ultra thick] (0.533,0.10) rectangle (0.90,0.90);
\node[green,ultra thick] at (0.72,0.6) {BDT1};
\draw[red,ultra thick] (0.20,0.50) rectangle (0.42,0.90);
\node[red,ultra thick] at (0.31,0.7) {BDT2};
\end{scope}
...
both stages of MVA are shown by the labelled boxes. The data falling below the
$\pi^0$ mass along the $\Delta_M$ axis in the left hand plot is a resolution
effect.}} \label{fig:trainingData}
\end{figure}
...
quality, as displayed in Fig.~\ref{bdt2TrainingVar}. 
...
The $D$ meson is required to be within $\pm$25 MeV/c$^2$ of the nominal $D^0$
mass, which corresponds to approximately three times the mass resolution. The
charged pion and kaon from the $B^{\pm}$ decay vertex are chosen to occupy the
kinematic range of the RICH detector, $5 < p < 100$ GeV/c and $0.5 < p_T < 10$
GeV/c. When a neutral pion is present, it is required to lie within the
asymmetric mass window of $110 < m[\pi^0] < 185$ MeV/c$^2$. 

\afterpage{
\clearpage % Flush earlier floats (otherwise order might not be
\begin{landscape}
\begin{table}[]
\centering
...
$\pi^0$, $\pi^0_{\gamma}$ refers to the partially reconstructed $\pi^0$ mode, where only
one of the photons from the $\pi^0$ is reconstructed in the final state.}}
